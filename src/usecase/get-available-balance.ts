...
@Service()
export class GetAvalableBalanceUseCase {
    ...
    async execute(bankAccountId: string): Promise<number>{
        ...
        const pendingCredit = await this.entryService.getNetPendingCredit(account.id)
        const postedDedit = await this.entryService.getPostedDebit(account.id)
        
        return postedDedit - (postedCredit + pendingCredit)
    }
}